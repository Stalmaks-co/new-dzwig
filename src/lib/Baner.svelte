...
<section class="relative overflow-hidden bg-black">
	...
	<div class="absolute inset-0 flex items-center justify-center text-white">
		...
		>
			{#if visible}
				<h1
					class="text-3xl leading-tight font-bold md:text-4xl lg:text-5xl xl:text-6xl 2xl:text-7xl"
					in:fly={{ y: -40, duration: 500, delay: 100, opacity: 0 }}
				>
					Wynajem dźwigów i podnośników
				</h1>
				<p
					class="mt-2 text-sm leading-snug sm:text-base md:text-lg lg:text-2xl xl:text-3xl 2xl:text-4xl"
					in:fly={{ y: 40, duration: 500, delay: 300, opacity: 0 }}
				>
					Profesjonalne usługi dźwigowe w Barlinku i okolicach
				</p>
				<button
					onclick={handleButtonClick}
					in:fly={{ y: 60, duration: 500, delay: 600, opacity: 0 }}
					class="bg-accent mt-6 flex flex-row items-center justify-center rounded-lg px-4 py-2 text-xs text-white transition-colors duration-200 hover:bg-red-700 focus:ring-2 focus:ring-red-500 focus:ring-offset-2 focus:outline-none sm:px-6 sm:py-3 sm:text-sm md:text-base lg:text-lg"
				>
