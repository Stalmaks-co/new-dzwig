...
<main class="relative z-10 mx-auto mb-8 min-h-[70vh] max-w-7xl px-4 sm:px-6 lg:px-8">
	...
	>
		<div class="rounded-t-lg p-8 md:w-2/3 md:rounded-l-lg">
			...
			<form method="POST" on:submit|preventDefault class="space-y-4">
				...
				<input
					...
					class="text-secondary focus:ring-accent mb-4 w-full rounded-lg border border-gray-300 bg-gray-50 p-2.5 text-base focus:ring-2 focus:ring-offset-2 focus:ring-offset-white focus:outline-none"
					required
				/>
				...
				></textarea>
				<button
					class="bg-accent xs:text-lg mx-auto flex w-full flex-row items-center justify-center rounded-md px-4 py-2 text-base text-white transition duration-300 hover:bg-red-700 focus:ring-2 focus:ring-red-500 focus:ring-offset-2 focus:outline-none sm:px-6 sm:py-3 sm:text-xl"
					on:click={sendForm}
				>
					<p>Wyślij</p>
				</button>
		...
		<div
			class="bg-secondary/10 flex flex-col justify-around overflow-hidden rounded-b-lg p-6 shadow-lg md:w-1/2 md:rounded-r-lg md:p-10"
		>
